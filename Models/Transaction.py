...
class LightTransaction():
    ...
    def execute_transactions():
        ...
        if p.useFIFO:
            pass
            # print("Transaction Ordering: FIFO")# pool is left as-is — it already reflects arrival order
        else:
            # print("Transaction Ordering: Fee-Priority")
            pool = sorted(pool, key=lambda x: x.fee, reverse=True)     
        # print("First 10 tx fees in selected order:", [round(tx.fee, 6) for tx in pool[:10]])
        # /////////////////////////////////////////////////////////////////////////
